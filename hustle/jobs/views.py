...
def view(request):
    if request.user.groups.filter(name="Worker").exists():
        open_jobs = Job.objects.filter(claimed_user=request.user, complete=False)
    ...
    return render(request=request, template_name="jobs/view_all.html", context={"open_jobs": open_jobs, "completed_jobs": completed_jobs, "isWorker": isWorker})
